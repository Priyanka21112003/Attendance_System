...
lecture_count = 1  # Start from Lecture 1
MAX_LECTURES = 5  # Set maximum lecture limit

# Function to read or initialize the lecture session
def get_current_lecture():
    global lecture_count
    today = time.strftime('%d_%m_%Y')
    session_file = f'Records/{today}/lecture_session.txt'
    
    # Check if today's session file exists, otherwise create it and reset lecture count
    if os.path.exists(session_file):
        with open(session_file, 'r') as f:
            lecture_count = int(f.read().strip())
    else:
        # Reset lecture count for a new day and create a new session file
...
# Function to update the lecture count for a new session
...
# Attendance function to create separate files for each lecture and mark attendance only once per lecture
def Attendance(name):
    ...
    # Create lecture-specific file within the date folder
    filename = f'{date_directory}/lecture_{current_lecture}.csv'

    # Check if the name has been recorded in the current lecture file
...
# Real-time face recognition handling
...
# Handle attendance marking for group photo
...
# Function to start a new lecture session with limit of 5 lectures
